...
type Handler struct {
	config Config
	// Fault is available only to the isolated pilot launcher.
...
// New validates enrollment and compiles the pinned local schema catalog.
func New(ctx context.Context, s *store.Store, config Config) (*Handler, error) {
	...
	h := &Handler{config: config, store: s, driver: s.GetDriver().(store.ShrimpDriver), key: key, origin: u.Scheme + "://" + u.Host, path: u.EscapedPath(), schemas: map[string]json.RawMessage{}, resolved: map[string]*jsonschema.Resolved{}}
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case r.Method == "GET" && strings.HasPrefix(path, "/operations/"):
		...
		result, err := h.driver.ShrimpResult(r.Context(), claims.Subject, parts[0], parts[1])
		if err != nil {
...
func (h *Handler) mutate(w http.ResponseWriter, r *http.Request, claims *accessClaims) {
	...
	m := store.ShrimpMutation{Principal: claims.Subject, Window: op["replay_window"].(string), ID: op["id"].(string), Fingerprint: hex.EncodeToString(hash[:]), Action: command["action"].(string), Deadline: deadline.Unix(), CommandID: command["command_id"].(string), RecoverOnly: !h.config.AllowWrite || !hasScope(claims, "shrimp.write")}
	...
	result, err := h.store.ApplyShrimp(ctx, m)
	if err != nil {
...
func (h *Handler) writeReceipt(w http.ResponseWriter, window, id string, result *store.ShrimpResult) {
	s := result.Subject
	resources := []any{}
	effects := []any{}
	state := "succeeded"
	...
	if result.Error != "" {
		...
		if result.Action == "disable" || result.Action == "retire" {
			effects = append(effects, map[string]any{"id": "admission", "kind": "admission_block", "resource": ref("subject", s.ID), "consumer": "memos-session-refresh-pat", "state": "complete", "deadline": stamp(result.Time), "observed_frontier": result.Token, "error": nil})
		}
	}
